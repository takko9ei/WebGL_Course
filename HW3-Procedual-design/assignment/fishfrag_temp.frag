...
void main() {
    ...
    vec3 color_body = getBodyPattern(centcoord,0.952,2.032); 
...
float scale_pattern(vec2 uv, float scale, float rot) {
...
vec3 getBodyPattern(vec2 uv, float scale, float rot){
    ...
    float d = scale_pattern(uv, scale, rot);
    ...
    vec3 iridescent_color = vec3(0.3, 0.2, 0.5) * sin(uv.y * 8.0 + u_time * 0.5);
